...
# --- SALARY SCALES (FULL RESTORATION) ---
...
def extract_file_data(file_obj):
    ...
    try:
        ...
        dm = re.search(r"Commission Statement for\s+([A-Za-z]+\s+\d{4})", norm_text, re.I)
        pm = re.search(r"Personnel Number:\s*(\d+)\s+([A-Za-z\s]+?)\s*(?:Position|Target|ZAR)", norm_text, re.I)
        tm = re.search(r"Target Commission:[^\d]*?([\d\.,]+\.\d{2})", norm_text, re.I)
...
prof_key = st.radio("Select Profile:", list(PROFILES.keys()), horizontal=True)
...
st.divider()
st.subheader("1. Identify & Scale Verification")
...
st.subheader("2. Stream Overrides (Swap buttons fix SABC errors)")
entries = []
visible_segments = list(PROFILES[prof_key]["statement"].keys())
for s in visible_segments:
    cols = st.columns([3, 2, 2, 1])
    cols[0].write(f"**{s}**")
    # Link widget directly to session state keys for the swap to reflect
    ...
    cols[3].button("🔴 Swap", key=f"btn_{s}", on_click=swap_act_tar, args=(s,))
...
if sap_active:
    ...
    r_sap, t_sap = SAP_DATA_25_26.get(month_match, (0.0, 0.0))
    sc1, sc2 = st.columns(2)
    sap_r = sc1.number_input(f"SAP Radio ({month_match})", value=r_sap)
    sap_t = sc2.number_input(f"SAP TV ({month_match})", value=t_sap)
...
if st.button("RUN FULL FORENSIC REPORT", type="primary", use_container_width=True):
    ...
    final_report += fmt_report("SCENARIO 1: SABC WEIGHTS (STMT MIDPOINT)", run_scenario(entries, m_stmt, PROFILES[prof_key]["statement"]))
    final_report += fmt_report("SCENARIO 2: SABC WEIGHTS (CURRENT SCALE)", run_scenario(entries, m_curr, PROFILES[prof_key]["statement"]))
    final_report += fmt_report("SCENARIO 5: POLICY WEIGHTS (CURRENT SCALE)", run_scenario(entries, m_curr, PROFILES[prof_key]["policy"], 'additive'))
    ...
    if prof_key == "Sports PM" and sap_active:
        ...
        final_report += fmt_report("SCENARIO 6: SAP RECONCILIATION (WITH DIGITAL)", run_scenario(sap_entries, m_curr, PROFILES[prof_key]["statement"]))
        sap_no_dig = [e for e in sap_entries if e["name"] != "Digital"]
        final_report += fmt_report("SCENARIO 7: SAP RECONCILIATION (EXCL. DIGITAL)", run_scenario(sap_no_dig, m_curr, {"Radio Sport Sponsorship": 30.0, "TV Sport Sponsorship": 60.0}))
    ...
    st.download_button("Download Report", data=pdf.output(dest='S').encode('latin-1'), file_name=fn)
