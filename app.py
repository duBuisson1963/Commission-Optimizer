...
st.divider()
st.subheader("1. Extract Identity & Verification")
ce1, ce2, ce3 = st.columns(3)
emp_name = ce1.text_input("Confirm Name:", value=st.session_state["emp_name"])
...
if st.button("RUN FULL FORENSIC COMPARISON", type="primary", use_container_width=True):
    ...
    final_report += build_rep("SCENARIO 1: SABC WEIGHTS (STMT MIDPOINT)", run_scenario(entries, m_stmt, PROFILES[prof_key]["statement"]))
    final_report += build_rep("SCENARIO 2: SABC WEIGHTS (CURRENT SCALE)", run_scenario(entries, m_curr, PROFILES[prof_key]["statement"]))
    final_report += build_rep("SCENARIO 5: POLICY WEIGHTS (CURRENT SCALE)", run_scenario(entries, m_curr, PROFILES[prof_key]["policy"], 'additive'))
    ...
    if prof_key == "Sports PM" and activate_sap:
        ...
        final_report += build_rep("SCENARIO 6: SAP RECONCILIATION (WITH DIGITAL)", run_scenario(sap_entries, m_curr, PROFILES[prof_key]["statement"]))
        sap_no_dig = [e for e in sap_entries if e["name"] != "Digital"]
        final_report += build_rep("SCENARIO 7: SAP RECONCILIATION (EXCL. DIGITAL)", run_scenario(sap_no_dig, m_curr, {"Radio Sport Sponsorship": 30.0, "TV Sport Sponsorship": 60.0}))
